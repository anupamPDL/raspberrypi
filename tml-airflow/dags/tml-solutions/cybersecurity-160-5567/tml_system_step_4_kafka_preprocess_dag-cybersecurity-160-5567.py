...
default_args = {
  'owner' : 'Anupam Poudel',  # <<< *** Change as needed      
  ...
  'jsoncriteria': 'uid=hostName,filter:allrecords~subtopics=hostName,hostName,hostName~values=inboundpackets,outboundpackets,pingStatus~identifiers=inboundpackets,outboundpackets,pingStatus~datetime=lastUpdated~msgid=~'
...
@dag(dag_id="tml_system_step_4_kafka_preprocess_dag_cybersecurity-160-5567", default_args=default_args, tags=["tml_system_step_4_kafka_preprocess_dag_cybersecurity-160-5567"], start_date=datetime(2023, 1, 1),schedule=None,catchup=False)
def startprocessing():
  def empty():
     pass
dag = startprocessing()
